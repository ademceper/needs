...
import { ArrowsClockwise as SyncAltIcon, DotsThreeVertical } from "@phosphor-icons/react";
...
const ButtonVariant = { primary: "default", secondary: "outline", link: "link" } as const;
type IRowCell = { title?: React.ReactNode };
type IAction = { title: React.ReactNode; onClick?: (...a: any[]) => void; onRowClick?: (row: any) => void; isDisabled?: boolean };
type IActions = IAction[];
type IActionsResolver = (row: any, extraData?: any) => IActions;
type IFormatter = (value: any, extraData?: any) => any;
type ITransform = (label?: any) => { className?: string };
type IRow = { data: any; cells?: any[]; disableActions?: boolean; disableSelection?: boolean; selected?: boolean; isOpen?: boolean };
type TableProps = { ariaLabelKey?: string; [k: string]: any };
const TableVariant = { compact: "compact" } as const;

function Th({ children, className, ...rest }: any) {
  return <TableHead className={className} {...rest}>{children}</TableHead>;
}
function Td({ children, className, ...rest }: any) {
  return <TableCell className={className} {...rest}>{children}</TableCell>;
}
function ActionsColumn({ items }: { items: IActions; extraData?: any }) {
  return (
    <DropdownMenu>
...
function ToolbarItem({ children, className }: any) {
